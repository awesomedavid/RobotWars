...
public class DisplayDuoTeam extends DisplayTeam implements DisplayValues{
	...
	public DisplayDuoTeam(Team team)
	{
		...
		yPos = CoreValues.RESOLUTION_Y-HUD_HEIGHT;
		
		if(team.getID() == 0)
		{
			xPos = DisplayValues.CARD_INDENT/2;
			resources = new DisplayDuoResources(team, CoreValues.RESOLUTION_X/2-225, yPos);
		}
		else
		{
			xPos = CoreValues.RESOLUTION_X - CARD_SPACING * 4; 
			resources = new DisplayDuoResources(team, CoreValues.RESOLUTION_X/2+180, yPos);
		}
	...
	public void renderName(Graphics g)
	{
		...
		if(team.getID() == 1)
		{
			x = x + CoreValues.RESOLUTION_X - w - indent;
		}
		else
		{
			x = x + indent;
