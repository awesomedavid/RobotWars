...
public class Minimap implements CoreValues{
	...
	Minimap()
	{
		...
		xPos = RESOLUTION_X - MAP_INDENT - MAP_BORDER - getMinimapWidth();
	...
	{		
		...
		g.drawRect(xPos + Camera.getX() + Camera.getX() * getMinimapWidth() / (World.getWidth() * CoreValues.CELL_SIZE), 
				   ...
				   getMinimapWidth() * RESOLUTION_X / (World.getWidth() * CoreValues.CELL_SIZE), 
				   getMinimapHeight() * RESOLUTION_Y / (World.getHeight() * CoreValues.CELL_SIZE));
